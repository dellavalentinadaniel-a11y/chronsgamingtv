...
export function PlatformPage() {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      setLoading(true);
      try {
        const data = await articleService.getArticlesByPlatform(platformName);
        setArticles(data);
      } catch (error) {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <SEO 
        title={`Videojuegos de ${platformName}`} 
        description={`Explora las últimas noticias, guías detalladas y análisis de los videojuegos más populares para la plataforma ${platformName} en ChronsGamingtv.`}
      />
